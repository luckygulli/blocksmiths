...
const provider = (new ethers.providers.Web3Provider(web3Provider))
    ...
    try {
        ...
        if (latestBlock && latestBlock.transactions.length > 0) {
            ...
            const lastTransactionHash = latestBlock.transactions[latestBlock.transactions.length - 1];

            // Fetch the full details of the transaction using its hash
            const transactionDetails = await provider.getTransaction(lastTransactionHash);
